...
public class EnrollmentProcessor {
	...
	protected void process() {
		readFile(filename);
		Collections.sort(enrollmentList, Comparator.comparing(Enrollment::getCompanyName)
				.thenComparing(Enrollment::getVersion).reversed().thenComparing(Enrollment::getId));
		HashSet<Object> seen = new HashSet<>();
		enrollmentList.removeIf(e -> !seen.add(Arrays.asList(e.getCompanyName(), e.getId())));
		Collections.sort(enrollmentList,
				Comparator.comparing(Enrollment::getCompanyName).thenComparing(Enrollment::getSortName));
		writeFiles();
	}
	...
	private void readFile(String fileName) {
		try {
			enrollmentList = new CsvToBeanBuilder<Enrollment>(new FileReader(filename)).withType(Enrollment.class)
					.build().parse();
		...
		while (!enrollmentList.isEmpty()) {
			...
			while (companyFlag) {
				try (Writer writer = new FileWriter(
						"output/" + company.replaceAll("\\s", "_") + "_enrollment.csv");) {
